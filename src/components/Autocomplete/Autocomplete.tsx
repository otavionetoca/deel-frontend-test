...
export const Autocomplete = React.memo(
  ({
    ...
    const filteredSuggestions = suggestions.filter((w) => w.includes(_value));
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
      switch (e.key) {
        case "Enter": {
          ...
          // a single item
          if (suggestionCursor < 0 && filteredSuggestions.length > 0) {
            handleSelectSuggestion(filteredSuggestions.at(0) || "");
            setSuggestionCursor(0);
            return;
          }

          const suggestionAtIndex = filteredSuggestions[suggestionCursor] || "";
        ...
        case "ArrowUp": {
          setSuggestionCursor((oldValue) =>
            oldValue - 1 < 0 ? filteredSuggestions.length - 1 : oldValue - 1
          );
          break;
        }
        ...
        case "ArrowDown": {
          setSuggestionCursor(
            (oldValue) =>
              (oldValue < filteredSuggestions.length - 1 && oldValue + 1) || 0
          );
          break;
        }
    ...
    const suggestionItems = useMemo(
          ...
          return (
            <li
              ...
              {...(suggestionCursor === idx
                ...
                : {})}
              ref={(suggestionCursor === idx && highlightedRef) || undefined}
            >
              {before}
              {(_value && (
                <span style={{ fontWeight: "bolder" }}>{_value}</span>
              )) ||
                null}
              {(after && after) || null}
            </li>
      ...
      [filteredSuggestions, _value, setSuggestionCursor, onSelectSuggestion]
    );
      ...
      <div className="autocomplete-container" onKeyDown={handleKeyDown}>
        ...
        {(_value.length > 0 && !hasSelectedSuggestion && (
          <ul>
            ...
            {(!isLoading && !suggestionItems.length && (
